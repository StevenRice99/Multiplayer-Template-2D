...
public class GameManager : NetworkManager
{
    ...
    /// </summary>
    public static float Move => Instance._optionsOpen ? 0 : Instance._move;
    ...
    private void HandleInteractions()
    {
        ...
        if (_optionsOpen || localPlayer == null)
        {
            return;
        }

        // Get the nearest object you can interact with in range.
        InteractBase interactBase = interactObjects.Where(i => Vector3.Distance(localPlayer.transform.position, i.transform.position) <= i.ActivationDistance).OrderBy(i => Vector3.Distance(localPlayer.transform.position, i.transform.position)).FirstOrDefault();
    ...
    private void HandleCamera()
    {
        ...
        // Set to the local player if there is one, keeping a Z offset.
        if (localPlayer != null)
        {
            Vector3 player = localPlayer.transform.position;
            player.z = camTransform.position.z;
            camTransform.position = player;
        }
